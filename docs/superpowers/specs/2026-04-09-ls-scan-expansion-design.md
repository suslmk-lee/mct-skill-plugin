...
│       ├── SKILL.md
│       ├── analyzers/
│       │   ├── phase1-rules.md      # Phase 1 규칙 정의
│       │   ├── phase2-llm.md        # Phase 2 LLM 분석 로직
│       │   └── comparator.md        # 이전 스캔 비교 로직
│       └── security/
│           └── auth.md              # 권한 검증 로직
│
└── .claude/
    ├── linear.json              # [필드 추가] enable_scan, scan_admin_user
    └── scan-results.json        # [신규] 스캔 결과 저장
```
